...
class Rates:
    ...
    def get_last_n_ticks(
        ...
        requested_data = mt5.copy_ticks_from(
            symbol, datetime.now(timezone.utc), n_ticks, ENUM_COPY_TICKS.COPY_TICKS_ALL
        )
        ...
        # Convert to DataFrame
        tick_data = pd.DataFrame(requested_data)

        # Convert timestamp to datetime
        tick_data["time"] = pd.to_datetime(tick_data.time, unit="s", utc=True)
        
        # Convert timestamp to datetime
        tick_data["time_msc"] = pd.to_datetime(tick_data.time_msc, unit="ms", utc=True)
        
        # Select only the useful columns
        tick_data = tick_data[["time", "bid", "ask", "time_msc"]]
    ...
    def get_specific_tick(
        ...
        # Request Tick data
        requested_data = mt5.copy_ticks_from(symbol, date_from, 1, ENUM_COPY_TICKS.COPY_TICKS_ALL)
    ...
    def get_ticks_range(
        ...
        Returns:
            pd.DataFrame: Requested range ticks data
        """
        ...
        requested_data = mt5.copy_ticks_range(
            symbol, date_from, date_to, ENUM_COPY_TICKS.COPY_TICKS_ALL
        )
    ...
    def validate_count_candles(cls, n_candles: int) -> None:
        ...
        if n_candles >= max_bars:
            raise ValueError(
                f"[ERROR]: The number of candles cannot be higher than terminal chart max bars({max_bars-1})"
            )
