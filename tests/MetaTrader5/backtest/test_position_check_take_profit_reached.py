...
from algo_trading.sources.MetaTrader5_source.backtest.backtest import (
    __backtest_position_check_take_profit_reached,  # Stop Loss Verification Function
...
# **Test 1: Position `BUY` Reaches Take Profit**
def test_buy_position_take_profit_hit(account: Account):
    """
    Tests if a `BUY` position is closed correctly when the candle's `high` price reaches the take profit.
    """
    ...
    # Opens a `BUY` position with take profit at 1.14
    ...
    ), "The `BUY` position should be open."

    # Verifies if the take profit was reached
    ...
    ), "The `BUY` position should have been closed by the take profit."


# **Test 2: Position `SELL` Reaches Take Profit**
def test_sell_position_take_profit_hit(account: Account):
    """
    Tests if a `SELL` position is closed correctly when the candle's `low` price reaches the take profit.
    """
    account.login_backtest(balance=10_000, leverage=100)
    operation_handler = account.backtest_account_data.operation
    ...
    # Opens a `SELL` position with take profit at 1.10
    __backtest_position_open(
        operation_class=operation_handler,
        symbol=symbol,
    ...
    assert (
        len(operation_handler.account_data.positions) == 0
    ), "The `SELL` position should have been closed by the take profit."


# **Test 3: Position `BUY` Does Not Reach Take Profit**
def test_buy_position_no_take_profit(account: Account):
    """
    Tests if a `BUY` position remains open when the candle's `high` price does not reach the take profit.
    """
    account.login_backtest(balance=10_000, leverage=100)
    operation_handler = account.backtest_account_data.operation
...
# **Test 4: Position `SELL` Does Not Reach Take Profit**
def test_sell_position_no_take_profit(account: Account):
    """
    Tests if a `SELL` position remains open when the candle's `low` price does not reach the take profit.
    """
    account.login_backtest(balance=10_000, leverage=100)
    operation_handler = account.backtest_account_data.operation
...
# **Test 5: Position Without Take Profit Configured**
def test_position_without_take_profit(account: Account):
    """
    Tests if a position remains open when there is no take profit configured.
    """
    account.login_backtest(balance=10_000, leverage=100)
    operation_handler = account.backtest_account_data.operation
    ...
    # Opens a `BUY` position without take profit
    __backtest_position_open(
        operation_class=operation_handler,
        symbol=symbol,
        order_type=ENUM_POSITION_TYPE.POSITION_TYPE_BUY,
    ...
    assert (
        len(operation_handler.account_data.positions) == 1
    ), "The `BUY` position should not have been closed, as there is no take profit configured."


# **Test 6: Position `BUY` with Take Profit Reached Exactly at the `high`**
def test_buy_position_take_profit_exact_high(account: Account):
    """
    Tests if a `BUY` position is closed correctly when the candle's `high` price is exactly equal to the take profit.
    """
    account.login_backtest(balance=10_000, leverage=100)
    operation_handler = account.backtest_account_data.operation
...
# **Test 7: Position `SELL` with Take Profit Reached Exactly at the `low`**
def test_sell_position_take_profit_exact_low(account: Account):
    """
    Tests if a `SELL` position is closed correctly when the candle's `low` price is exactly equal to the take profit.
    """
    account.login_backtest(balance=10_000, leverage=100)
    operation_handler = account.backtest_account_data.operation
...
# **Test 8: Position `BUY` with Take Profit Not Hit (`high` Below the TP)**
def test_buy_position_take_profit_not_hit(account: Account):
    """
    Tests if a `BUY` position remains open when the candle's `high` price is below the take profit.
    """
    account.login_backtest(balance=10_000, leverage=100)
    operation_handler = account.backtest_account_data.operation
    ...
    # Opens a `BUY` position with take profit at 1.13
    __backtest_position_open(
        operation_class=operation_handler,
        symbol=symbol,
        order_type=ENUM_POSITION_TYPE.POSITION_TYPE_BUY,
...
# **Test 9: Position `SELL` with Take Profit Not Hit (`low` Above the TP)**
def test_sell_position_take_profit_not_hit(account: Account):
    """
    Tests if a `SELL` position remains open when the candle's `low` price is above the take profit.
    """
    account.login_backtest(balance=10_000, leverage=100)
    operation_handler = account.backtest_account_data.operation
    ...
    # Opens a `SELL` position with take profit at 1.11
    __backtest_position_open(
        operation_class=operation_handler,
        symbol=symbol,
        order_type=ENUM_POSITION_TYPE.POSITION_TYPE_SELL,
...
# **Test 10: Multiple Positions with Take Profit**
def test_multiple_positions_take_profit(account: Account):
    """
    Tests the behavior with multiple positions opened with different take profits.
    """
    account.login_backtest(balance=10_000, leverage=100)
    operation_handler = account.backtest_account_data.operation
    ...
    # Opens multiple positions with different take profits
    __backtest_position_open(
        operation_class=operation_handler,
        symbol=symbol,
        order_type=ENUM_POSITION_TYPE.POSITION_TYPE_BUY,
    ...
    __backtest_position_open(
        ...
        order_type=ENUM_POSITION_TYPE.POSITION_TYPE_BUY,
        volume=1,
        profit_price=1.17,  # Should not be reached
        comment="Test Buy Position TP 3",
    )

    # Verifies if the take profits were reached
    __backtest_position_check_take_profit_reached(operation_handler)

    # Verifies if only the positions with take profit reached were closed
    assert (
        len(operation_handler.account_data.positions) == 1
    ), "Only the `Test Buy Position TP 3` position should remain open."
