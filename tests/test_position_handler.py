...
def handler():
    ...
    return OperationHandler(initial_balance=10000)

@pytest.fixture
def reset_handler(handler: OperationHandler):
    """Fixture to reset handler after each test."""
    yield
    handler.reset()

def test_create_position(handler: OperationHandler, reset_handler):
...
def test_close_position(handler: OperationHandler, reset_handler):
    """Testa o fechamento de uma posição."""
    handler.create_order(OrderType.BUY, price=100, quantity=10, sl=95, tp=110)
    handler._execute_order(handler.pending_orders[0])
    
    position = handler.positions[0]
    handler.close_position(price=105, quantity=10, reason="Manual close", position=position)
...
def test_partial_close_position(handler: OperationHandler, reset_handler):
    """Testa o fechamento parcial de uma posição."""
    handler.create_order(OrderType.BUY, price=100, quantity=10, sl=95, tp=110)
    handler._execute_order(handler.pending_orders[0])

    position = handler.positions[0]
    handler.close_position(price=105, quantity=5, reason="Partial close", position=position)
...
def test_multiple_partial_closes(handler: OperationHandler, reset_handler):
...
def test_full_close_after_partial(handler: OperationHandler, reset_handler):
    ...
    position = handler.positions[0]
    handler.close_position(price=105, quantity=5, reason="Partial close", position=position)
    handler.close_position(price=108, quantity=5, reason="Final close", position=position)
...
def test_take_profit_trigger(handler: OperationHandler, reset_handler):
...
def test_stop_loss_trigger(handler: OperationHandler, reset_handler):
...
def test_modify_position(handler: OperationHandler, reset_handler):
...
def test_statistics(handler: OperationHandler, reset_handler):
    """Testa o cálculo de estatísticas após operações."""
    
    handler.create_order(OrderType.BUY, price=100, quantity=10, sl=95, tp=110)
    handler._execute_order(handler.pending_orders[0])
    handler.close_position(price=105, quantity=10, reason="Manual close", position=handler.positions[0])
...
def test_close_position_excess_quantity(handler: OperationHandler, reset_handler):
    ...
    with pytest.raises(ValueError, match="Cannot close more than the available quantity."):
        handler.close_position(price=105, quantity=15, reason="Excess close", position=position)

def test_invalid_position_parameters(handler: OperationHandler, reset_handler):
...
def test_cancel_order(handler: OperationHandler, reset_handler):
...
def test_order_timeout(handler: OperationHandler, reset_handler):
    
    handler.create_order(OrderType.BUY, price=100, quantity=10, max_time_active=5)
    order = handler.pending_orders[0]
    
    handler._process_pending_orders(current_price=90, current_time=pd.Timestamp.now() + pd.Timedelta(seconds=10))
    
    assert order.status == OrderStatus.CANCELLED
    assert len(handler.pending_orders) == 0

def test_order_not_executed(handler: OperationHandler, reset_handler):
